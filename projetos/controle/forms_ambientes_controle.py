...
def ambientes_form(ambientes: dict):
    ...
    ambientes = dict(reversed(list(ambientes.items())))
...
def campos_ambientes_calculados_esquadrias(area_ambiente, indicadores, esquadrias, zona_bioclimatica, regiao):
    ...
    if len(indicadores) > 0:
        ...
        # Soma das areas das janelas
        soma_area_janelas = esquadrias['area_total_vidro'].sum()
        ...
        for index, indicador_ambiente in enumerate(indicadores):
            ...
            # Area de influencia
            area_vidro = esquadrias[esquadrias['indicador'] == indicador_ambiente]['area_total_vidro'].values[0]
            area_influencia = (area_ambiente/soma_area_janelas)*area_vidro

            # Area janela
            coef_vidro = esquadrias[esquadrias['indicador'] == indicador_ambiente]['coef_vidro'].values[0]
            area_janela = area_influencia/coef_vidro

            # proporção
            altura = esquadrias[esquadrias['indicador'] == indicador_ambiente]['altura'].values[0]
            largura = esquadrias[esquadrias['indicador'] == indicador_ambiente]['largura'].values[0]
            proporcao = min(altura, largura)/max(altura, largura)

            # Lados
            lado_maior2 = (area_janela/proporcao)*0.5
            lado_menor2 = lado_maior2*proporcao

            # Variação maior
            variacao_maior = (lado_maior2-max(altura, largura))/2
        ...
        esquadrias['Area*coef abertura'] = esquadrias['area']*esquadrias['coef_abertura']
        ...
        children_areas_ponderadas_abertura = html.H6(f'Soma das areas ponderadas pelo coeficiente de abertura: {soma_areas_ponderadas_abertura}m². Situação: {situacao_abertura_ventilacao}', className='mt-4', id={'type': 'situacao_abertura_ventilacao', 'index': f'{indicador_ambiente}'}) 

        # Areas ponderadas pelos coefs de vidro
        esquadrias['Area*coef vidro'] = esquadrias['area']*esquadrias['coef_vidro']
        ...
        children_areas_ponderadas_vidro = html.H6(f'Soma das areas ponderadas pelo coeficiente de vidro: {soma_areas_ponderadas_vidro}m². Situação: {situacao_elementos_transparentes}', id={'type': 'situacao_elementos_transparentes', 'index': f'{indicador_ambiente}'}) 
