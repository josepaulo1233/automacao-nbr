            ...
            dmc.GridCol(
                children=[
                    ...
                    dcc.Upload(
                        ...
                        #html.Div([
                        #    'Arraste ou selecione o arquivo VN',
                        #]),
                        # style={'border': 'dashed'}  # você pode reabilitar seu estilo aqui
                    ),
            ...
            dmc.GridCol(
                children=[
                    ...
                    dcc.Upload(
                        ...
                        # html.Div([
                        #     'Arraste ou selecione o arquivo HVAC',
                        # ]),
                        # style={'border': 'dashed'}  # você pode reabilitar seu estilo aqui
                    ),
...
# Input('ventilacao', 'value'),
)
def ajusta_arquivo_VN(arquivo_vn, txt_file_vn, esquadrias):
    ...
    if arquivo_vn is not None:
        ...
        if len(sections_less) == 0: 
            output_file_name = txt_file_vn.split('.')[0] + '_FINAL.' + txt_file_vn.split('.')[1]
...
def baixar_arquivo(data, txt_file_vn, n_clicks):

    ctx = dash.callback_context

    if ctx.triggered_id == "btn-download-vn":
        outputfile_name = txt_file_vn.split('.')[0] + '_FINAL.' + txt_file_vn.split('.')[1]
...
Input('upload-data-hvac', 'contents'),
Input('upload-data-hvac', 'filename'),     
)
def ajusta_arquivo_HVAC(arquivo_hvac, txt_file_hvac):

    if arquivo_hvac is not None:

        # Abrindo o arquivo como dataframe
        start_time = time.time()

        _, content_string = arquivo_hvac.split(',')
        decoded = base64.b64decode(content_string)
        ...
        output_file_name = txt_file_hvac.split('.')[0] + '_HVAC.' + txt_file_hvac.split('.')[1]
        output_file_name = './outputs/' + output_file_name
...
def baixar_arquivo_hvac(data, txt_file_vn, n_clicks):

    ctx = dash.callback_context

    if ctx.triggered_id == "btn-download-hvac":

        outputfile_name = txt_file_vn.split('.')[0] + '_HVAC.' + txt_file_vn.split('.')[1]
        return dict(content=data, filename=outputfile_name, type='text/plain')
