...
from uploads.uploads import get_upload_component
        ...
        dbc.Row([
            ...
            dbc.Col(
                children=[
                    ...
                    get_upload_component(id='upload-csv_temp-real'),
                    dcc.Store(id='upload-csv_temp-real-data', data={}),
            ...
            dbc.Col(
                children=[
                    ...
                    get_upload_component(id='upload-csv_temp-referencia'),
                    dcc.Store(id='upload-csv_temp-referencia-data', data={}),
...
def create_table(df1, df2, tipo: str):
    ...
    conteudo = html.Div([
        ...
        dbc.Row([
            ...
            #     ),
            # ),
            get_upload_component(id=f'upload-csv_hvac-{tipo}'),
            dcc.Store(id=f'upload-csv_hvac-{tipo}-data', data={}),
...
@callback(
    ...
    Input('upload-csv_temp-real-data', 'data'),
    # Input('upload-csv_temp-real', 'contents'),
    # Input('upload-csv_temp-real', 'filename'),
    Input('tipologia', 'value'),
    )
def resultados_csv_real(data, TIPOLOGIA):

    if data:

        # Transformando o JSON armazenado no dcc.Store em um DataFrame
        data = pd.DataFrame(data)
        
        df1, df2 = gera_df_final(data, TIPOLOGIA)
...
@callback(
    ...
    Input('upload-csv_temp-referencia-data', 'data'),
    # Input('upload-csv_temp-referencia', 'filename'),
    Input('tipologia', 'value'),
)
def resultados_csv_referencia(data, TIPOLOGIA):

    if data:

        df = pd.DataFrame(data)

        df1, df2 = gera_df_final(df, TIPOLOGIA)
...
def resultados_csv_hvac_real(data_temp, data_hvac, TIPOLOGIA):
    ...
    if data_temp and data_hvac:
        ...
        data_hvac = pd.DataFrame(data_hvac)
        
        df1, df2 = gera_df_temperatura_e_cargatermica(data_temp, data_hvac, TIPOLOGIA)
...
@callback(
    ...
    Input('upload-csv_temp-referencia-data', 'data'),
    Input('upload-csv_hvac-referencia-data', 'data'),
